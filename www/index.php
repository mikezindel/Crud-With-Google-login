...
function renderProduct($nId){
    ...
    if ($nId<6)
    {
    $oStmt = $oDb->prepare("SELECT * FROM products WHERE groupID=:id");
    $oStmt->bindParam("id", $nId);
    $oStmt->execute();
    $aProducts = $oStmt->fetchAll(PDO::FETCH_OBJ);    
    ...
    elseif($nId<8)
    {
    $oStmt = $oDb->prepare("SELECT * FROM products WHERE groupID=1");
    $oStmt->execute();
    $aProducts = $oStmt->fetchAll(PDO::FETCH_OBJ);

    //fetching offers
    $oStmt = $oDb->prepare("SELECT * FROM offers WHERE productID =1");
    $oStmt->execute();
    $aOffers = $oStmt->fetchAll(PDO::FETCH_OBJ);
    }
    elseif($nId<9)
    {
    $oStmt = $oDb->prepare("SELECT * FROM products WHERE groupID=2");
    $oStmt->execute();
    $aProducts = $oStmt->fetchAll(PDO::FETCH_OBJ);
     //fetching offers
    $oStmt = $oDb->prepare("SELECT * FROM offers WHERE productID =2");
    $oStmt->execute();
    $aOffers = $oStmt->fetchAll(PDO::FETCH_OBJ);
    }
    elseif($nId<10)
    {
    $oStmt = $oDb->prepare("SELECT * FROM products WHERE groupID=3");
    $oStmt->execute();
    $aProducts = $oStmt->fetchAll(PDO::FETCH_OBJ);
     //fetching offers
    $oStmt = $oDb->prepare("SELECT * FROM offers WHERE productID =3");
    $oStmt->execute();
    $aOffers = $oStmt->fetchAll(PDO::FETCH_OBJ);
    }
    elseif($nId<11)
